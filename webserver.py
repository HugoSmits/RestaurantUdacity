        ...
        try:
            ...
            if self.path.endswith("/edit"):
                ...
                if myRestaurantQuery:
                    ...
                    output += "</form>"
                    output += "</body></html>"
                    # input name is the Name of the variable. One that will be called
                    self.wfile.write(output)
    ...
    def do_POST(self):
        try:
            if self.path.endswith("/delete"):
                ...
                myRestaurantQuery = session.query(Restaurant).filter_by(
                    id=restaurantIDPath).one()
                # Why doesnt delete have a non null verification ? after the query ?
                if myRestaurantQuery:
            ...
            if self.path.endswith("/restaurants/new"):
                ...
                if ctype == 'multipart/form-data':
                    ...
                    fields = cgi.parse_multipart(self.rfile, pdict)
                    #Returns a dictionary just like urlparse.parse_qs() 
                    #keys are the field names, each value is a list of values for that field.
                    messagecontent = fields.get('newRestaurantName')
                    # get(Key) returns value
                    
                    # No if is needed since there is no query 
